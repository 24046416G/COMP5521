...
class Transaction {
    ...
    check() {
        ...
        // 添加对新交易类型的验证
        if (this.type === Config.TRANSACTION_TYPE.STUDENT_REGISTRATION) {
            this.checkStudentRegistration();
        } else if (this.type === Config.TRANSACTION_TYPE.ATTENDANCE) {
    ...
    checkStudentRegistration() {
        ...
        // 验证接收方是否为教师地址
        if (output.address !== Config.ATTENDANCE_CONFIG.DEFAULT_TEACHER_ADDRESS) {
            throw new TransactionAssertionError('Student registration must be sent to teacher address');
        }

        // 验证metadata
        if (!output.metadata || 
            !output.metadata.studentId ||
            !output.metadata.classId ||
            !output.metadata.registrationTime ||
            !output.metadata.studentAddress) {
            throw new TransactionAssertionError('Invalid student registration metadata');
        }
    }
    ...
    checkAttendance() {
        ...
        // 验证接收方是否为教师地址
        if (output.address !== Config.ATTENDANCE_CONFIG.DEFAULT_TEACHER_ADDRESS) {
            throw new TransactionAssertionError('Attendance must be sent to teacher address');
        }

        // 验证metadata
        if (!output.metadata || 
            !output.metadata.studentId ||
            !output.metadata.courseId ||
            !output.metadata.timestamp ||
            !output.metadata.attendanceType ||
            !output.metadata.studentAddress) {
            throw new TransactionAssertionError('Invalid attendance metadata');
        }
    }
